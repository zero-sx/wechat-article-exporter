  ...
  <div class="flex flex-col h-full">
    ...
    <div class="flex flex-1 overflow-hidden">
      ...
      <ul class="flex flex-col h-full w-fit overflow-y-scroll divide-y">
        <li
          v-for="accountInfo in sortedAccountInfos"
        ...
        >
          ...
          <p>
            ID: <span class="font-mono">{{ accountInfo.fakeid }}</span>
          </p>
          <UBadge
            variant="subtle"
            color="green"
            class="absolute top-4 right-2"
            >{{ accountInfo.articles }}</UBadge
          >
        </li>
      </ul>
      ...
      <main class="flex-1 h-full overflow-y-scroll">
        ...
        <div class="relative" v-else-if="selectedAccount">
          <div
            class="sticky top-0 z-50 bg-white flex justify-between items-center px-4 h-[40px]"
          >
            <div class="flex items-center space-x-4">
              ...
              <UPopover :popper="{ placement: 'bottom-start' }">
                <UButton icon="i-heroicons-calendar-days-20-solid" color="gray">
                  {{ format(query.dateRange.start, "yyyy-MM-dd") }} -
                  {{ format(query.dateRange.end, "yyyy-MM-dd") }}
                </UButton>
              ...
              <UButton color="gray" variant="solid" @click="search"
                >搜索</UButton
              >
            </div>
            <div class="flex items-center space-x-2">
              ...
              <UButton
                ...
                class="disabled:bg-slate-4 disabled:text-slate-12"
                :disabled="
                  selectedArticles.length === 0 || batchDownloadLoading
                "
                @click="doBatchDownload"
              >
                <Loader
                  v-if="batchDownloadLoading"
                  :size="20"
                  class="animate-spin"
                />
                <span v-if="batchDownloadLoading"
                  ...
                  >
                  <span v-if="batchDownloadPhase === '打包'"
                    >{{ batchPackedCount }}/{{ batchDownloadedCount }}</span
                  >
                </span>
                <span v-else>下载HTML</span>
              </UButton>
              <UButton
                color="black"
                variant="solid"
            ...
            <tbody>
              ...
              >
                ...
                <td class="text-center">
                  {{
                    article.copyright_stat === 1 && article.copyright_type === 1
                      ? "原创"
                      : ""
                  }}
                </td>
          ...
          >
            <span class="text-green-500"
              ...
              {{ displayedArticles.length }}</span
            >
            <span class="text-rose-300" v-if="deletedArticlesCount > 0"
              >已隐藏 {{ deletedArticlesCount }} 条删除文章</span
            >
          </div>
        </div>
      </main>
...
async function switchTableData(fakeid: string) {
  ...
  deletedArticlesCount.value = data.filter(
...
function isRangeSelected(duration: Duration) {
  return (
    isSameDay(query.dateRange.start, sub(new Date(), duration)) &&
    isSameDay(query.dateRange.end, new Date())
  )
}
...
function search() {
  ...
  articles.forEach((article) => {
    ...
    if (
      ...
      !query.authors.includes(article.author_name)
    ) {
      article.display = false
    }
    if (
    ...
    if (
      query.isOriginal === "非原创" &&
      article.copyright_stat === 1 &&
    ...
    if (
      ...
      article.appmsg_album_infos.every(
        (album) => !query.albums.includes(album.title)
      )
    ) {
      article.display = false
    }
...
function doBatchDownload(downloadType: string = "zip") {
  const articles: DownloadableArticle[] = selectedArticles.value.map(
    (article) => ({
      title: article.title,
      url: article.link,
  ...
  batchDownload(articles, filename, downloadType)
}
...
async function exportToExcel(data: AppMsgEx[]) {
  ...
  data.forEach((item) => {
    worksheet.addRow({
      ...
      copyright:
        item.copyright_stat === 1 && item.copyright_type === 1 ? "原创" : "",
      album: item.appmsg_album_infos
        .map((album) => "#" + album.title)
        .join(" "),
      digest: item.digest,
      link: item.link,
      cover_img:
        item.pic_cdn_url_235_1 ||
        item.pic_cdn_url_16_9 ||
        item.cover_img ||
        item.cover,
