...
import type { DownloadableArticle, Article, Album } from "~/types/types"
...
async function downloadAssetWithProxy<T extends Blob | string>(
  ...
  let targetURL = proxy
    ? `${proxy}?url=${encodeURIComponent(url)}&headers=${encodeURIComponent(
...
export async function downloadArticleHTMLs(
  ...
  const htmlDownloadFn = async (
    article: DownloadableArticle,
    proxy: string
  ) => {
    const fullHTML = await downloadAssetWithProxy<string>(
      article.url,
      proxy,
      true
    )
...
export async function packHTMLAssets(html: string, title: string, zip?: JSZip) {
  ...
  if (
    pubTimeMatchResult &&
    pubTimeMatchResult.groups &&
    pubTimeMatchResult.groups.date
  ) {
    __setPubTime(
      parseInt(pubTimeMatchResult.groups.date),
  ...
  const ipWordingMatchResult = html.match(
    /window\.ip_wording = (?<data>{\s+countryName: '[^']+',[^}]+})/s
  )
  if (
    ipWrp &&
    ipWording &&
    ipWordingMatchResult &&
    ipWordingMatchResult.groups &&
    ipWordingMatchResult.groups.data
  ) {
  ...
  const titleModifiedMatchResult = html.match(
    /window\.isTitleModified = "(?<data>\d*)" \* 1;/
  )
  if (
    titleModifiedMatchResult &&
    titleModifiedMatchResult.groups &&
    titleModifiedMatchResult.groups.data
  ) {
  ...
  if (js_share_source && contentTpl) {
    const html = contentTpl.innerHTML
      ...
      .replace(
        /<mpvoice([^>]*?)js_editor_audio([^>]*?)><\/mpvoice>/g,
        "<p>[语音]</p>"
      )
      .replace(
        /<mpgongyi([^>]*?)js_editor_gy([^>]*?)><\/mpgongyi>/g,
        "<p>[公益]</p>"
      )
      .replace(
        /<qqmusic([^>]*?)js_editor_qqmusic([^>]*?)><\/qqmusic>/g,
        "<p>[音乐]</p>"
      )
      .replace(
        /<mpshop([^>]*?)js_editor_shop([^>]*?)><\/mpshop>/g,
        "<p>[小店]</p>"
      )
      .replace(
        /<iframe([^>]*?)class=[\'\"][^\'\"]*video_iframe([^>]*?)><\/iframe>/g,
        "<p>[视频]</p>"
      )
      .replace(
        /(<iframe[^>]*?js_editor_vote_card[^<]*?<\/iframe>)/gi,
        "<p>[投票]</p>"
      )
      .replace(
        /<mp-weapp([^>]*?)weapp_element([^>]*?)><\/mp-weapp>/g,
        "<p>[小程序]</p>"
      )
  ...
  // 下载留言数据
  let commentHTML = ""
  const commentIdMatchResult = html.match(
    /var comment_id = '(?<comment_id>\d+)' \|\| '0';/
  )
  if (
    commentIdMatchResult &&
    commentIdMatchResult.groups &&
    commentIdMatchResult.groups.comment_id
  ) {
    ...
    if (commentResponse) {
      ...
      commentResponse.elected_comment.forEach((comment) => {
        commentHTML +=
          '<div style="margin-top: 25px;"><div style="display: flex;">'
        ...
        commentHTML +=
          '<div style="flex: 1;"><p style="display: flex;line-height: 16px;margin-bottom: 5px;">'
        ...
        commentHTML += `<span style="font-size: 12px;color: #b5b5b5;">${formatAlbumTime(
          comment.create_time
        )}</span>`
        commentHTML +=
          '<span style="flex: 1;"></span><span style="display: inline-flex;align-items: center;">'
        commentHTML += `<span class="sns_opr_btn sns_praise_btn" style="font-size: 12px;color: #8b8a8a;">${
          comment.like_num || ""
        }</span>`
        commentHTML += "</span></p>"
        ...
        if (
          comment.reply_new.reply_total_cnt -
            comment.reply_new.reply_list.length >
          0
        ) {
          commentHTML +=
  ...
  // 阅读量
  let readNum = -1
  const readNumMatchResult = html.match(
    /var read_num = ['"](?<read_num>\d+)['"] \* 1;/
  )
  const readNumNewMatchResult = html.match(
    /var read_num_new = ['"](?<read_num_new>\d+)['"] \* 1;/
  )
  if (
    readNumNewMatchResult &&
    readNumNewMatchResult.groups &&
    readNumNewMatchResult.groups.read_num_new
  ) {
    readNum = parseInt(readNumNewMatchResult.groups.read_num_new, 10)
  } else if (
    readNumMatchResult &&
    readNumMatchResult.groups &&
    readNumMatchResult.groups.read_num
  ) {
  ...
  if ($js_image_desc) {
    ...
    const qmtplMatchResult = html.match(
      /(?<code>window\.__QMTPL_SSR_DATA__\s*=\s*\{.+?)<\/script>/s
    )
    if (
      qmtplMatchResult &&
      qmtplMatchResult.groups &&
      qmtplMatchResult.groups.code
    ) {
      ...
      $jsArticleContent
        .querySelector("#js_top_profile")!
        .classList.remove("profile_area_hide")
    }
    const pictureMatchResult = html.match(
      /(?<code>window\.picture_page_info_list\s*=.+\.slice\(0,\s*20\);)/s
    )
    if (
      pictureMatchResult &&
      pictureMatchResult.groups &&
      pictureMatchResult.groups.code
    ) {
      const code = pictureMatchResult.groups.code
      eval(code)
      const picture_page_info_list = (window as any).picture_page_info_list
      const containerEl = $jsArticleContent.querySelector(
  ...
  // 视频分享消息
  const $js_common_share_desc = $jsArticleContent.querySelector(
  ...
  if ($js_common_share_desc) {
    ...
    if (
      videoContentMatchResult &&
      videoContentMatchResult.groups &&
      videoContentMatchResult.groups.value
    ) {
      const code =
        "window.videoContentNoEncode = " + videoContentMatchResult.groups.value
      eval(code)
  ...
  const $js_mpvedio = $jsArticleContent.querySelector(
    ".js_video_channel_container > #js_mpvedio"
  )
  if ($js_mpvedio) {
    // 分享视频
    // poster
    let poster = ""
    const mpVideoCoverUrlMatchResult = html.match(
      /(?<code>window\.__mpVideoCoverUrl\s*=\s*'[^']*';)/s
    )
    if (
      mpVideoCoverUrlMatchResult &&
      mpVideoCoverUrlMatchResult.groups &&
      mpVideoCoverUrlMatchResult.groups.code
    ) {
    ...
    // video info
    let videoUrl = ""
    const mpVideoTransInfoMatchResult = html.match(
      /(?<code>window\.__mpVideoTransInfo\s*=\s*\[.+?];)/s
    )
    if (
      mpVideoTransInfoMatchResult &&
      mpVideoTransInfoMatchResult.groups &&
      mpVideoTransInfoMatchResult.groups.code
    ) {
      ...
      if (Array.isArray(mpVideoTransInfo) && mpVideoTransInfo.length > 0) {
        ...
        const resourceDownloadFn = async (url: string, proxy: string) => {
          const videoData = await downloadAssetWithProxy<Blob>(
            url,
            proxy,
            false,
            10
        ...
        const div = document.createElement("div")
        div.style.cssText =
          "height: 381px;background: #000;border-radius: 4px; overflow: hidden;margin-bottom: 12px;"
        div.innerHTML = `<video src="${videoURLMap.get(
          videoUrl
        )}" poster="${videoURLMap.get(
          poster
  ...
  // 下载内嵌音频
  const mpAudioEls =
    $jsArticleContent.querySelectorAll<HTMLElement>("mp-common-mpaudio")
  if (mpAudioEls.length > 0) {
    const audioResourceDownloadFn = async (
      asset: AudioResource,
      proxy: string
    ) => {
      const audioData = await downloadAssetWithProxy<Blob>(
        asset.url,
        proxy,
        false,
        10
  ...
  if (
    videoPageInfosMatchResult &&
    videoPageInfosMatchResult.groups &&
    videoPageInfosMatchResult.groups.code
  ) {
    ...
    videoPageInfos.forEach((videoPageInfo) => {
      ...
      if (
        videoPageInfo.is_mp_video === 1 &&
        videoPageInfo.mp_video_trans_info.length > 0
      ) {
    ...
    const videoIframes = $jsArticleContent.querySelectorAll(
      "iframe.video_iframe"
    )
    videoIframes.forEach((videoIframe) => {
      ...
      if (mpvid) {
        ...
        if (videoInfo) {
          ...
          div.innerHTML = `<video src="${videoURLMap.get(
            videoInfo.mp_video_trans_info[0]?.url
          )}" poster="${videoURLMap.get(
            videoInfo.cover_url
          )}" controls style="width: 100%;height: 100%;"></video>`
        ...
        const vidMatchResult = src.match(
          /v\.qq\.com\/iframe\/preview\.html\?vid=(?<vid>[\da-z]+)/i
        )
        if (
          vidMatchResult &&
          vidMatchResult.groups &&
          vidMatchResult.groups.vid
        ) {
  ...
  // 处理自定义组件
  const hasMpAudio =
    $jsArticleContent.querySelector("mp-common-mpaudio") !== null
  ...
  if (hasMpAudio) {
    ...
    customElementTemplate +=
      '<script type="text/javascript" src="./assets/mp-common-mpaudio.js"></script>'
...
export async function resolveArticleHTML(articles: DownloadableArticle[]): Promise<Article[]> {
  const results = []
  for (const article of articles) {
    ...
    // 优先从meta标签获取作者信息，不存在则回退到#js_name
    const author = doc.querySelector('meta[name="author"]')?.getAttribute('content') ||  ""
    const description = doc.querySelector('meta[name="description"]')?.getAttribute('content') ||  ""
    const accountName = doc.querySelector('#js_name')?.textContent || ""
    const copyrightLogo = doc.querySelector('#copyright_logo')?.textContent || ""
    ...
    let publishTime = doc.querySelector('#publish_time')?.getAttribute('content') || ""
    // 如果未提取到 publishTime，应从 html 中提取时间戳或格式化时间
    if (!publishTime) {
      const timestampMatch = html.match(/var oriCreateTime = '(?<timestamp>\d+)'/)
      const formattedMatch = html.match(/var createTime = '(?<formatted>\d{4}-\d{2}-\d{2} \d{2}:\d{2})';/)
      
      if (timestampMatch?.groups?.timestamp) {
        publishTime = dayjs.unix(parseInt(timestampMatch.groups.timestamp)).format('YYYY-MM-DD HH:mm:ss')
      } else if (formattedMatch?.groups?.formatted) {
        publishTime = formattedMatch.groups.formatted
      }
    }
    const readNum = doc.querySelector('#js_read_num')?.textContent || "-1"

    // 获取合集
    const publicTagInfoMatch = html.match(/var publicTagInfo\s*=\s*(\[.+?\]);/s)
    const albumList: Album[] = []
    if (Array.isArray(publicTagInfoMatch) && publicTagInfoMatch.length > 0) {
      try {
        const getPublicTagInfo = new Function(`${publicTagInfoMatch[0]} return publicTagInfo`)
        const publicTagInfo = getPublicTagInfo()
        for (const tag of publicTagInfo) {
          albumList.push({
            id: tag.id,
            title: tag.title,
            size: tag.size ? parseInt(tag.size) : null,
            link: tag.link,
            type: tag.type,
            continousReadOn: tag.continous_read_on
          })
        }
      } catch (e) {
        console.error('解析publicTagInfo失败:', e)
      }
    }
    // 去掉所有换行符、制表符、空格
    ...
    results.push({
      url,
      accountName: formatText(accountName),
